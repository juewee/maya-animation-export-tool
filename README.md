...
匹配到**多个**候选（真的重名）时，导出过程中会**直接弹窗**让你选：

- 候选 ≤3 个：系统选择框，一个候选一个按钮 + 「取消（跳过）」
- 候选更多：列表弹窗，选中后点「使用选中的节点」
- **选择会写回条目**（条目里的物体名更新成选中节点），之后导出不再询问；也可以随时用
  条目行上的「更新」按钮手动改指
- 点「取消（跳过）」则该条目本次跳过；批处理（mayapy）下没有 UI，退回警告 + 跳过

另外，**导出只要有失败就会弹窗提醒**（标题「导出未全部完成」，列出失败条目和原因），
不会再只丢一条脚本编辑器警告让你错过。
...
- 导出结束若**有失败**或**中途取消**，会弹一个「导出未全部完成」的对话框：成功/失败数量 +
  失败条目类型、名称、原因（最多列 8 条，其余见脚本编辑器）
- 同名节点选择弹窗（见上一节）也是同一个目的：遇到命名空间重名时当场选，不用事后翻日志
